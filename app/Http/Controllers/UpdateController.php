    ...
    */
    ...
    public function get(Request $request) {
        if($request->user()->role === 'super') {
            ...
            return redirect()->route('home')->with('error', trans('setup.message.error.unauthorised'));
        }
    }
    ...
    protected function install() {
        ...
        return redirect('update/' . ($current_version === $initial_version ? '0' : '1'));
    }
    ...
    protected function load($status) {
        return redirect()->route('settings.index')->with('success', trans($status === '0' ? 'setup.message.success.same' : 'setup.message.success.update', array(
