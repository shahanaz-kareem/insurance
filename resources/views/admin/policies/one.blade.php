...
@section('policy_actions')
                    <div class="ui right floated successish button top m-w-140 right pointing dropdown" data-inverted="" data-tooltip="{{ trans('policies.menu.header.tooltip') }}" data-position="left center">
                        ...
                        <div class="menu">
                            ...
                            <form action="{{ route('policies.delete', array($policy->id)) }}" class="item negative" method="POST">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                <a href="#" class="delete">
        ...
        <div class="scrolling content">
            ...
            <form action="{{ route('policies.edit', array($policy->id)) }}" method="POST">
                ...
                <div class="ui form">
                    ...
                    <div class="field">
                        <label>{{ $custom_field->label }}</label>
                        <textarea name="custom_fields[C{{ $policy->client->company->id }}F{{ $custom_field->id }}][value]" {{ isset($custom_field->required) ? ' required' : '' }} rows="1">{{ $custom_field->default }}</textarea>
                    </div>
                        @endif
                    @endforeach
                </div>
