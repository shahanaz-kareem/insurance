...
<body class="auth">
  <div class="auth-card">
    <div class="auth-branding">
      <!-- <img src="{{ 'http://iworksync.com/ebs/insurance/storage/app/images/' . config('insura.favicon') }}"> -->
      <div class="auth-branding">
      <img src="{{ asset('uploads/images/' . config('insura.favicon')) }}">
    </div>
    </div>
    <div class="sign-in" style="{{ $state === 'login' ? '' : 'display:none;' }}">
      <h3>Insurance - Policy Manager</h3>
...
<form action="{{ route('password.email') }}" method="POST">
    ...
    <div class="reset" style="">
      ...
      <form action="{{ route('password.update') }}" method="POST">
        ...
        <div class="ui form">
          <div class="field required">
            <label>{{ trans('auth.input.label.email') }}</label>
            <input type="email" name="email" placeholder="{{ trans('auth.input.placeholder.email') }}" required value="{{ $email ?? old('email') }}"/>
          </div>
          <div class="field required">
    ...
    <div class="activate" style="">
      ...
      <form action="{{ route('password.activate') }}" method="POST">
        {!! csrf_field() !!}
    ...
    <div style="text-align:center;">
      <div class="button dropdown selection floating fluid icon labeled pointed search ui">
        ...
        <div class="menu">
          ...
          <a class="item" href="{{ route('auth.show', array('language' => $language['locale'])) }}">{{ $language['name'] }}</a>
          @endforeach
        </div>
      </div>
    </div>
  ...
  <script src="{{ asset('assets1/libs/semantic-UI/semantic.min.js') }}"></script>
  <script src="{{ asset('assets1/js/insura.js') }}" type="text/javascript"></script>
  <!-- Page Specific Scripts -->
  <script src="{{ asset('assets1/js/auth.js') }}"></script>
  <!-- Extra Scripts -->
