(function($) {
    window.insura = {
        helpers: {
            ...
            initTelInput: function(selector) {
                (typeof selector === 'string' ? $(selector) : selector).blur(function(e) {
                    ...
                    utilsScript: '{{ asset('assets1/libs/intl-tel-input/js/utils.js') }}'
                });
            },
            listenForChats: function() {
                ...
                if(typeof(EventSource) !== "undefined") {
                    var es = new EventSource('{{ route('chats.live', ['quiet' => true]) }}');
                    ...
                    (function longPoll() {
                        $.get('{{ route('chats.live', ['quiet' => true]) }}', null, null, 'json').done(function(result) {
        ...
        urls: {
            ...
            chats: {
                live: '{{ route('chats.live') }}',
                see: '{{ route('chats.see') }}',
                send: '{{ route('chats.send') }}'
            }
        }
    };
})(window.jQuery);
