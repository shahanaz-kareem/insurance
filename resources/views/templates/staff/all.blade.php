...
@section('content')
        ...
        <div class="row clients-list">
                ...
                <div class="ui segment white client-list-card">
                    <div class="client-list-about">
                        <div class="client-list-avatar">
                            ...
                            @else
                            <img src="{{ 'http://iworksync.com/ebs/insurance/storage/app/images/users/' . $employee->profile_image_filename }}" alt="{{ $employee->first_name }} {{ $employee->last_name }}"/>
                            @endif
                        </div>
                        <div class="client-list-info">
                            ...
                            <div class="client-list-contact">
                                ...
                                <div class="col-xs-6 col-sm-6 col-md-6 text-ellipsis">
                                    <i class="ion-ios-telephone"></i> {{ $employee->phone ?? '(---) ---- --- ---' }}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row client-list-footer">
                        <div class="col-xs-3 col-sm-4 col-md-4">
                            <p>
                                <!-- <strong>{{ trans('staff.label.commission') }}:</strong> {{ $employee->currency_symbol ?? $staff->currency_symbol }}{{ $employee->commission }} -->
                                <strong>Staff Code:</strong> {{ $employee->staff_code}}
                            </p>
                        </div>
                        <!-- <div class="col-xs-5 col-sm-4 col-md-4">
                            <p>
                                ...
                                <span class="text-danger">{{ $employee->currency_symbol ?? $staff->currency_symbol }}{{ $employee->due }}</span>
                                @elseif ($employee->sales === $employee->paid)
                                <span class="text-success">{{ trans('staff.label.status.paid') }}</span>
                                @else
                                <span class="text-info">{{ $employee->currency_symbol ?? $staff->currency_symbol }}{{ $employee->due }}</span>
                                @endif
                        ...
                        <div class="col-xs-5 col-sm-4 col-md-4">
                            <p>
                                <strong>Branch:</strong> @foreach($branches as $branch)<?php if($branch->id==$employee->branch_id){echo $branch->branch_name;}?> @endforeach
                            </p>
                        </div>
                        <div class="col-xs-4 col-sm-4 col-md-4 text-right client-list-more">
                            <a href="{{ route('staff.one', array($employee->id)) }}" class="mini ui button"> {{ trans('staff.link.profile') }} </a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-12 text-center">
                {!! $staff->links('vendor.pagination.semantic-ui') !!}
            </div>
        </div>
@endsection
