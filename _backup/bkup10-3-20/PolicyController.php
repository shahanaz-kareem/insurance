...
class PolicyController extends Controller {
    ...
    */
    ...
    public function getAll(Request $request) {
        ...
        switch($user->role) {
            case 'super':
                $view_data['policies'] = $user->company->policies()->with('branch')->insuraFilter($filters)->paginate(15);
            ...
            case 'admin':
            case 'staff':
                $view_data['policies'] = $user->company->policies()->with('branch')->where('branch_id',$request->user()->branch_id)->insuraFilter($filters)->paginate(15);
        ...
        $view_data['policies']->transform(function($policy) {
            ...
            if($policy->renewal=='0000-00-00') {
                $policy->renewal='-';
            } else {
                if($policy->type=='annually'){
                    if($policy->renewal_date < date('Y-m-d')){

                        $renewal_date = date('Y-m-d', strtotime('+1 years', strtotime($policy->renewal)));
                        $data = DB::table('policies')
                                ->where('id', $policy->id)
                                ->update(['renewal_date' => $renewal_date]);
                    ...
                    if($policy->renewal_date < date('Y-m-d')){
                        $renewal_date = date('Y-m-d', strtotime('+1 months', strtotime($policy->renewal)));
                        $data = DB::table('policies')
                                ->where('id', $policy->id)
                                ->update(['renewal_date' => $renewal_date]);
                        $policy->renewal = date('d-m-Y', strtotime($policy->renewal_date));
                    ...
                    if($policy->renewal_date < date('Y-m-d')){
                        $renewal_date = date('Y-m-d', strtotime('+6 months', strtotime($policy->renewal)));
                        $data = DB::table('policies')
                                ->where('id', $policy->id)
                                ->update(['renewal_date' => $renewal_date]);
                        $policy->renewal = date('d-m-Y', strtotime($policy->renewal_date));
                    ...
                    if($policy->renewal_date < date('Y-m-d')){
                        $renewal_date = date('Y-m-d', strtotime('+3 months', strtotime($policy->renewal)));
                        $data = DB::table('policies')
                                ->where('id', $policy->id)
                                ->update(['renewal_date' => $renewal_date]);
                        $policy->renewal = date('d-m-Y', strtotime($policy->renewal_date));
